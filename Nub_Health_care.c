...
void load_data() {
    ...
    if (file) {
        ...
        for (int i = 0; i < employe_count; i++) {
            char post_lower[MAX_NAME_LENGTH];
            strcpy(post_lower, employes[i].post);
            to_lowercase(post_lower);
            if (strcmp(post_lower, "doctor") == 0) {
...
// Add a patient from an employee
void add_patient_from_Employe(long id) {
    ...
    printf("Employee added successfully as patient!\n");
}
...
void add_employe() {
    ...
    printf("Enter post: ");
    scanf(" %49[^\n]", new_employe.post);
    to_lowercase(new_employe.post);
    printf("Enter blood group: ");
    scanf("%9s", new_employe.blood_group);
    to_lowercase(new_employe.blood_group);
    printf("Enter age: ");
...
// Search for an employee by ID and print details
...
// View doctor details for appointment
void doctor_view_appointmentView(long id) {
    ...
    for (int i = 0; i < employe_count; i++) {
        ...
        strcpy(post_lower, employes[i].post);
        to_lowercase(post_lower);
        if (employes[i].id == id && strcmp(post_lower, "doctor") == 0) {
...
// Remove a patient based on employee ID
void remove_patient_asPer_employe(long id) {
...
// Remove a doctor based on employee ID
void remove_doctor_asPer_employe(long id) {
...
void add_patient() {
    ...
    to_lowercase(new_patient.blood_group);
    printf("Enter age: ");
...
// Search for a patient by ID and print details
...
// Search for patients by blood group
void search_patient_and_print_forBlood(char blood_group[]) {
    char input_blood_group[MAX_BLOOD_GROUP_LENGTH];
    strcpy(input_blood_group, blood_group);
    to_lowercase(input_blood_group);

    for (int i = 0; i < patient_count; i++) {
        char patient_blood_group[MAX_BLOOD_GROUP_LENGTH];
        strcpy(patient_blood_group, patients[i].blood_group);
        to_lowercase(patient_blood_group);

        if (strcmp(patient_blood_group, input_blood_group) == 0) {
            printf("\nID: %ld\n", patients[i].id);
            printf("Name: %s\n", patients[i].name);
            printf("Department: %s\n", patients[i].department);
            printf("Occupation: %s\n", patients[i].occupation);
...
void blood_request() {
    ...
    for (int i = 0; i < donation_count; i++) {
        ...
        if (index != -1) {
            char patient_blood_group[MAX_BLOOD_GROUP_LENGTH];
            strcpy(patient_blood_group, patients[index].blood_group);
            to_lowercase(patient_blood_group);

            if (strcmp(patient_blood_group, blood_group) == 0) {
                printf("Blood found:\n\n");
                printf("ID: %ld\n", patients[index].id);
                printf("Name: %s\n", patients[index].name);
                printf("Department: %s\n", patients[index].department);
...
void view_appointments() {
    ...
    for (int i = 0; i < appointment_count; i++) {
        ...
        for (int j = 0; j < employe_count; j++) {
            char post_lower[MAX_NAME_LENGTH];
            strcpy(post_lower, employes[j].post);
            to_lowercase(post_lower);
            if (employes[j].id == appointments[i].doctor_id && strcmp(post_lower, "doctor") == 0) {
                printf("Doctor Name: %s\n", employes[j].name);
                printf("Doctor ID: %ld\n", employes[j].id);
...
void prescription(int index) {
    ...
    printf("Enter Test Names (separated by commas): ");
    scanf(" %99[^\n]", test);
    while (getchar() != '\n');
...
void Add_Medicine() {
    ...
    scanf(" %49[^\n]", new_medicine.name);
    to_lowercase(new_medicine.name);
    ...
    for (int i = 0; i < medicine_count; i++) {
        char existing_name[MAX_NAME_LENGTH];
        strcpy(existing_name, medicine[i].name);
        to_lowercase(existing_name);
        if (strcmp(existing_name, new_medicine.name) == 0) {
...
void Search() {
    ...
    for (int i = 0; i < medicine_count; i++) {
        ...
        strcpy(medicine_name, medicine[i].name);
        to_lowercase(medicine_name);
        if (strcmp(medicine_name, search_name) == 0) {
...
void Billing() {
    ...
    while (1) {
        ...
        for (int i = 0; i < medicine_count; i++) {
            ...
            strcpy(medicine_name, medicine[i].name);
            to_lowercase(medicine_name);
            if (strcmp(medicine_name, billing_name) == 0) {
                found = 1;
...
void add_Test() {
    ...
    scanf(" %99[^\n]", new_test.name);
    to_lowercase(new_test.name);
    printf("Enter test price: ");
    scanf("%f", &new_test.price);

    for (int i = 0; i < test_count; i++) {
        char existing_name[100];
        strcpy(existing_name, tests[i].name);
        to_lowercase(existing_name);
        if (strcmp(existing_name, new_test.name) == 0) {
...
void search_Test() {
    ...
    for (int i = 0; i < test_count; i++) {
        char test_name[100];
        strcpy(test_name, tests[i].name);
        to_lowercase(test_name);
...
void test_billing() {
    ...
    while (1) {
        ...
        for (int i = 0; i < test_count; i++) {
            ...
            strcpy(test_name, tests[i].name);
            to_lowercase(test_name);
            if (strcmp(test_name, billing_name) == 0) {
                found = 1;
                price = tests[i].price;
                total_price += price;
